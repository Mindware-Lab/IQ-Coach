...
export async function bootstrapStudio(root:HTMLElement):Promise<()=>void> {
  ...
  function render() {
    ...
    if(screen==='briefing') {
      ...
      const invariant=node==='attention'?(wr==='C'?'Ignore the face. Use the arrow majority.':wr==='B'?'Find the IN / OUT majority in the motion.':wr==='AB_MIXED'?'Find the IN / OUT majority in arrows or motion.':'Find the IN / OUT majority in the arrows.'):'Keep the same operation as the presentation changes.';
