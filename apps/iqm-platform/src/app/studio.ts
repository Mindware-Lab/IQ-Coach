...
import {renderAttentionInstruction} from '../modules/attention/instruction';
...
export async function bootstrapStudio(root:HTMLElement):Promise<()=>void> {
  ...
  function render() {
    ...
    if(screen==='move') {
      ...
      frame(`<section class="st-move-page">${heading('The move','A simple rule. A deliberate choice.','The aim is to recognise the operation outside the game.')}<div class="st-move-grid"><div class="st-move-art" aria-hidden="true"><span></span><span></span><span></span><i></i></div><div class="st-step-list">${steps.map(([i,t,c],n)=>`<article><span>${icon(i)}</span><div><small>0${n+1}</small><h3>${t}</h3><p>${c}</p></div></article>`).join('')}</div></div><div class="st-takeaway"><strong>${escape(mod?.strategy.handle||w.question)}</strong><p>A strategy for the right moment — not a rule to apply everywhere.</p></div></section>`,{showNav:false,back:'home',action:button('When should I use it?','examples')});return;
    }
    if(screen==='examples') {
